...
export function showToast(type: ToastType, opts: ToastOptions): void {
  ...
  toast.setAttribute(
    "role",
    normalizedType === "error" || normalizedType === "failure"
      ? "alert"
      : "status",
  );
...
if (typeof window !== "undefined") {
  (window as unknown as { aiHubToast: typeof showToast }).aiHubToast =
    showToast;
}
...
function initCountryCombobox(
  ...
  const options = Array.from(
    dropdown.querySelectorAll<HTMLLIElement>(".country-option"),
  );
  ...
  function filterOptions(query: string) {
    ...
    let noResults = dropdown.querySelector(
      ".country-no-results",
    ) as HTMLElement | null;
    if (visibleCount === 0) {
      if (!noResults) {
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (visibleOpts.length > 0) {
        focusedIndex =
          (focusedIndex - 1 + visibleOpts.length) % visibleOpts.length;
        visibleOpts.forEach((o, i) =>
          o.classList.toggle("is-focused", i === focusedIndex),
        );
        input.setAttribute(
...
export function initParticipation() {
  ...
  const interests = Array.from(
    form.querySelectorAll<HTMLInputElement>('input[name="interest"]'),
  );
  ...
  const countryToggleBtn = form.querySelector<HTMLButtonElement>(
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (emailInput) {
      ...
      if (!em) {
        ...
        emailInput.setCustomValidity(
          "Enter a valid email address (e.g. name@organisation.com).",
        );
      } else {
        emailInput.setCustomValidity("");
      }
    ...
    if (orgInput) {
      orgInput.setCustomValidity(
        orgInput.value ? "" : "Enter your organisation.",
      );
    }
    ...
    interests.forEach((input) => input.setCustomValidity(""));

    if (consentInput) {
      consentInput.setCustomValidity(
        consentInput.checked
          ? ""
          : "Please confirm that you agree to the privacy notice before submitting.",
      );
    }
    ...
    const utm = {
      source:
        urlParams.get("utm_source") || urlParams.get("source") || "website",
    ...
    try {
      const endpoint = form.dataset.endpoint || "/api/register";
      ...
      const successTitle = isDuplicate
        ? "Already registered"
      ...
      if (submitButton) {
        ...
        submitButton.textContent = isDuplicate
          ? "Already registered ✓"
